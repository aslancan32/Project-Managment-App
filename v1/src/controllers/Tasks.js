...
const showTask = (req,res)=> {
    ...
    findOne({_id: req.params.taskId}, true)
...
const update = (req,res) => {
    if(!req.params?.taskId.match(/^[0-9a-fA-F]{24}$/)) return res.status(httpStatus.BAD_REQUEST).send({message: "ID is not correct"})
    //! Check Id isExist
    modify(req.params.taskId, req.body)
        .then((updatedDoc) => {
...
const addComment = (req,res) => {
    ...
    findOne({_id: req.params.taskId}).then((matchedObject) => {
...
export {create, showTask, update, deleteSection, addComment, addSubTask}